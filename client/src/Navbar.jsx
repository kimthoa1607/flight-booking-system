                ...
                <nav className='hidden sm:flex flex-rows w-full bg-hero-img box-content py-4 px-10'>
                    <div className='flex flex-row w-full justify-between '>
                        ...
                        <nav className='flex flex-row gap-5 text-white font-medium text-lg'>
                            <NavLink to='/flights' end className={({ isActive, isPending }) => isPending ? "pendinglink" : isActive ? "activelink" : "pendinglink"}>Chuyến Bay</NavLink>
                            ...
                            >Liên Hệ Chúng Tôi</NavLink>
                        </nav>
                    </div>
                </nav>
                <nav
                    className={`sm:hidden fixed max-h-screen z-50 inset-y-0 left-0 top-0 w-64 bg-blue-950 overflow-y-auto transition duration-500 transform ${isOpen ? 'translate-x-0 ease-in' : ' transform -translate-x-full ease-out'
                        }`}>
                    <button
